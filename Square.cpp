...
Square::Optional Square::fromCoordinates(Coordinate file, Coordinate rank) {
    if (file >= 8 || rank >= 8) return std::nullopt;
...
Square::Optional Square::fromName(const std::string& name) {
    if (name.size() != 2 || (name[0] > 'h' || name[0] < 'a') || (name[1] > '8' || name[1] < '1')) return std::nullopt;
    int file = name[0] - 'a';
    int rank = name[1] - '1';
    return { Square(file + rank * 8) };
}
...
std::uint64_t Square::getBitPresent() const {
    int i = index();
    int tr = (i / 8) * 8 + (7 - i % 8);
    return 1ull << tr;
}
...
std::ostream& operator<<(std::ostream& os, const Square& square) {
    return os << (char)('a' + square.file()) << square.rank() + 1;
}
